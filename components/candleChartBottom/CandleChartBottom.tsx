...
import EarningsWidget from './Earnings';
                ...
                <View className="relative">
                    ...
                    <Svg width={size} height={size / 2 + 20} viewBox={`0 0 ${size} ${size / 2 + 20}`}>
                        ...
                        <Path
                            d={createArcPath(180, 360, radius, centerX, centerY)}
                            stroke="#181E27"
                            strokeWidth={strokeWidth - 4} //black color wheel
                            fill="none"
                        />

                        {/* Left side - Strong Sell + Sell (Blue) */}
                        <Path
                            d={createArcPath(180, 240, radius, centerX, centerY)}
                            stroke="#D5674E"
                            strokeWidth={strokeWidth}
                            fill="none"
                            strokeLinecap="round"
                        ...
                        {/* Center - Neutral (Cyan/Teal) */}
                        <Path
                            d={createArcPath(240, 280, radius, centerX, centerY)}
                            stroke="#FCABC2"
                            strokeWidth={strokeWidth}
                            fill="none"
                            strokeLinecap="round"
        ...
        <View className="flex-1 px-2 mt-8 mb-[12%]">
            <View>
                ...
                <LinearGradient
                    ...
                    style={{ borderRadius: 8, margin: 0, borderWidth: 1, borderColor: '#374151', marginTop: 12, marginBottom: 12 }}
                >
                    <View className=" p-4 mb-4">
                        <Text className="text-white text-lg font-semibold mb-4">Key Stats</Text>
                ...
                <LinearGradient
                    colors={['#373941', '#18191D', '#18191D']}
                    style={{ borderRadius: 8, margin: 0, borderWidth: 1, borderColor: '#374151', marginTop: 12, marginBottom: 24 }}
                >
                ...
                <LinearGradient
                    colors={['#373941', '#18191D', '#18191D']}
                    style={{ borderRadius: 8, margin: 0, borderWidth: 1, borderColor: '#374151', marginBottom: 24 }}
                >

                    <View className=" p-4 mb-4">
                        <Text className="text-white text-lg font-semibold mb-2">Analyst rating</Text>

                        <TechnicalGaugeTwo />

                        <View className="mb-6">
                            {renderDataRow('Next earning report', 'July 30, 2025')}
                            {renderDataRow('Report period', 'Q2 2025')}
                            {renderDataRow('EPS estimate', '$173.38K USD')}
                            {renderDataRow('Revenue estimate', '$173.38 B')}
                        </View>
                ...
                <View className="border-b border-gray-800 p-4 mb-6">
                    <Text className="font-ivy text-white text-xl mb-2">Employees</Text>
                    {renderDataRow('Empoyees FY', '2.06K')}
                    {renderDataRow('Revenue/Employee (1Y)', '26.85K USD')}
                    {renderDataRow('Net Income/Employee (1Y)', '173.38K USD')}
                </View>

                {/* About Data Points */}
                <View className="p-4 mb-12">
                    <Text className="font-ivy text-white text-xl mb-2">About</Text>

                    {renderDataRow('Sector', 'Customers Durables')}
                    {renderDataRow('Revenue/Employee (1Y)', 'Motor Vehicles')}
                    {renderDataRow('C.E.O.', 'Elon Musk')}
                    {renderDataRow('Website', 'tesla.com')}
                    {renderDataRow('Headquarters', 'Austin')}
                    {renderDataRow('Founded', '2003')}
                    {renderDataRow('FIGI', 'DORA1602DSNRHN')}

                    <View className='relative'>
                        <Text className='text-gray-400 mt-2'>
                            Tesla, Inc. is an American multinational automotive and clean energy company. Headquartered in Austin, Texas, it designs, manufactures and sells battery electric vehicles, stationary energy storage systems, and renewable energy solutions.
                        </Text>

                        <BlurView intensity={10} tint={'systemMaterialDark'} style={[
                            styles.textBlurContainer,
                        ]} className='absolute top-10 flex justify-center items-center h-full w-full'>
                            <TouchableOpacity className='w-[50%]'>
                                <LinearGradient
                                    ...
                                    style={{ opacity: 0.67, borderRadius: 8, borderWidth: 1, borderColor: '#A1A6BF' }}
                                >
                                    <Text className="text-white text-sm font-medium mr-2">Continue Reading</Text>
                                </LinearGradient>
                            </TouchableOpacity>
                        </BlurView>
                    </View>
